...
namespace RatKing {
	...
	public partial class Bloed : EditorWindow {
		...
		[MenuItem("Window/Bloxels/Online Documentation")]
		static void OpenDocumentation() {
			Application.OpenURL("https://docs.google.com/document/d/1zGZsuFeGVjL8Hhq7pC56tL9RCN9ExowRraotjf0cM40/edit?tab=t.0");
		}
		...
		public bool MousecastChunks(Event evt, out RaycastHit hit, out BloxelChunk chunk, bool forceAll = false) {
			...
			if (!forceAll && pickCurrentLevelOnly) {
				...
				if (BloxelUtility.TryGetPrefabStage(out var prefabStage)) {
					...
					hitChunks.AddRange(GameObject.FindObjectsByType<BloxelChunk>(FindObjectsSortMode.None));
#else
		...
		void OnSceneGUI_Bloxels(SceneView view) {
			...
			if (curTool.AllowsPickingLevel && TryPickLevel(evt)) {
			}
			else if (allowClick) {
		...
		void Hotkeys_Bloxels(Event evt) {
			...
			tools[curToolIdx].Hotkeys(this, evt);
			
			if (evt.keyCode == KeyCode.None && evt.character == '\t') { evt.Use(); }
			...
			// focus
			if (evt.keyCode == KeyCode.F && BloxelLevel.Current != null) {
				evt.Use();
				void Do(SceneView v) {
					if (v == null) { return; }
					if (evt.shift) { v.pivot = Vec3TrPos(new Vector3(0.5f, 0.5f, 0.5f)); }
					else if (CurPos != null) { v.pivot = Vec3TrPos(CurPos.Value.ToVector() + new Vector3(0.5f, 0.5f, 0.5f)); }
				}
				if (SceneView.currentDrawingSceneView != null) { Do(SceneView.currentDrawingSceneView); }
				else { foreach (var v in SceneView.sceneViews) { Do(v as SceneView); } }
			}
