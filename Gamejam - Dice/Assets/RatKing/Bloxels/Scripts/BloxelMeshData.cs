...
namespace RatKing.Bloxels {
	...
	public class BloxelMeshData {
		...
		public List<Vector3> uvs; // third coord is for texture array index
		...
		static readonly BloxelTemplate[] tmpBlxTemplate = new BloxelTemplate[27];
		static readonly float[] tmpBlxNoisStr = new float[27];
		static readonly float[] tmpBlxNoisFac = new float[27];
		static readonly Vector3[] tmpNoisLerpPos = new Vector3[8];
		...
		public BloxelMeshData(int estimatedSize, bool hasUVs) {
			...
			if (hasUVs) { uvs = ListPool<Vector3>.Create(estimatedSize); }
			submeshCount = 1;
			triangles = new List<TriangleList> { new TriangleList(0, estimatedSize * 2) };
			vertexCount = 0;
		}
		...
		public BloxelMeshData(Mesh mesh, bool withUVs) {
			...
			// does not have third UV param
			if (withUVs) { uvs = ListPool<Vector3>.Create(mesh.vertexCount); mesh.GetUVs(0, uvs); }
		...
		public virtual void Dispose() {
			...
			vertexCount = 0;
			if (uvs != null) { ListPool<Vector3>.Dispose(ref uvs); }
		...
		public void AddTriangle(int meshIdx, int v0, int v1, int v2) {
			...
			if (submeshCount > meshIdx) {
				for (int i = triangles.Count - 1; i >= 0; --i) {
					if (triangles[i].index == meshIdx) {
						tris = triangles[i];
						break;
					}
				}
		...
		public BloxelMeshData(BloxelMeshData other, Handling trisHandling, Handling uvsHandling) {
			...
			if (uvsHandling == Handling.DeepCopy && other.uvs != null) {
				uvs = ListPool<Vector3>.Create(other.uvs.Count);
		...
		/// <summary>
		/// this version of AddTo doesn't need the direction and is thus a bit slower?
		...
		public void AddTo(BloxelMeshData other, BloxelTexture origTex, BloxelTexture tex, int extraRot, int offX, int offY, BloxelChunk chunk, Vector3 pos, ref int vertexCount, Bloxel.BuildMode buildMode) {
			...
			if (origTex.noiseStrength <= 0f || buildMode == Bloxel.BuildMode.CollisionEditor) {
				for (int i = 0; i < this.vertexCount; ++i) { // TODO optimize
					...
					var texIdx = texY * tex.countX + texX;
					var texA = tex.pos[texIdx];
					other.vertices.Add(vertices[i] + pos);
					other.normals.Add(normals[i]);
					if (buildMode == Bloxel.BuildMode.Render && uvs != null && other.uvs != null) {
						var uv = (texA + Vector2.Scale(GetRotatedUV(uvs[i], extraRot), tex.size)).XY_(tex.arrayIdx[texIdx]);
						other.uvs.Add(uv);
					}
				}
			}
			...
			for (int i = other.triangles.Count - 1; i >= 0; --i) {
				if (other.triangles[i].index == meshIdx) {
					for (int t = 0; t < tc; ++t) {
						other.triangles[i].indices.Add(tris[t] + vertexCount);
					}
					break;
				}
			}
		...
		public void ChangeUVsOf(IList<Vector3> meshUVs, BloxelTexture tex, int extraRot, int offX, int offY, Vector3 worldPos, ref int vertexCount) {
			var wp = worldPos;
			switch (rot) {
				case 1: wp = new Vector3(-wp.z, wp.y, wp.x); break;
				case 2: wp = new Vector3(-wp.x, wp.y, -wp.z); break;
			...
			for (int i = 0; i < this.vertexCount; ++i, ++vertexCount) {
				...
				if (uvs != null) {
					meshUVs[i] = (texA + Vector2.Scale(GetRotatedUV(uvs[i], extraRot), tex.size)).XY_(tex.arrayIdx[texIdx]);
				}
			}
		}

		// called by side data
		public void ChangeUVsOf(IList<Vector3> meshUVs, BloxelTexture tex, int extraRot, int offX, int offY, int dir, Vector3 worldPos, ref int vertexCount) {
			if (uvs == null) { return; }
		...
		public void ChangeUVsOf(IList<Vector3> meshUVs, int subMesh, BloxelTexture tex, int extraRot, Transform transform, int startTri = 0, int triCount = -1) {
			var worldPos = transform.position;

			if (preferDirs == null) {
				if (uvs != null) {
					for (int i = 0; i < this.vertexCount; ++i) {
						meshUVs[i] = (tex.pos[0] + Vector2.Scale(uvs[i], tex.size)).XY_(tex.arrayIdx[0]);
					}
				}
				return;
			}
			...
			for (int t = startTri; t < triCount; t += 3) {
				...
				if (uvs != null) {
					...
					for (int ii = 0; ii < 3; ++ii) {
						...
						var texIdx = texY * tex.countX + texX;
						meshUVs[i] = (tex.pos[texIdx] + Vector2.Scale(uvs[i], tex.size)).XY_(tex.arrayIdx[texIdx]);
					}
				}
			}
			//}
		...
		public void ReduceData() {
			...
			var newUVs = uvs != null ? ListPool<Vector3>.Create(trisCount / 2) : null;
		...
		public void GenerateUVs_BoxMapping(/*int dir, int rot, */Transform transform, int preferredDir = -1, float addAngle = 0f) {
			...
			var uvArray = new Vector3[vertexCount];
			for (int i = 0; i < vertexCount; ++i) {
		...
		public void ApplyUVsFrom(BloxelMeshData from, Handling handling) {
			...
			if (handling == Handling.DeepCopy) {
				if (uvs == null) { uvs = ListPool<Vector3>.Create(vertexCount); }
				else { uvs.Clear(); }
		...
		void PrepareUVs() {
			dir = rot = 0;
			if (uvs == null) { uvs = ListPool<Vector3>.Create(vertexCount); } else { uvs.Clear(); }
