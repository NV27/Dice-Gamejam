...
namespace RatKing.Bloxels {
	...
	public class BloxelTextureEditor : Editor {
		...
		SerializedProperty propCutInTexArray;
		...
		void OnEnable() {
			...
			propCutInTexArray = serializedObject.FindProperty(nameof(BloxelTexture.cutInTexArray));
		...
		public override void OnInspectorGUI() {
			...
			TextureAtlasSettings curTexAtlas = null;
			...
			if (BloxelUtility.ProjectSettings == null || serializedObject.targetObject == BloxelUtility.ProjectSettings.MissingTexture) {
				...
				curTexAtlas = BloxelUtility.ProjectSettings.TexAtlases[propTexAtlasIdx.intValue];
				...
				if (propTexture?.objectReferenceValue is Texture tex && !tex.isReadable) {
					GUI.color = Color.red;
					EditorGUILayout.ObjectField(propTexture, new GUIContent("Base Texture"));
					GUILayout.Label("Texture not set to Read/Write enabled!");
					if (GUILayout.Button("Open Texture Settings")) { Selection.SetActiveObjectWithContext(propTexture.objectReferenceValue, null); }
					GUILayout.Space(6f);
					GUI.color = color;
				}
				else {
			...
			if (serializedObject.isEditingMultipleObjects) {
				...
				if (bt.texture != null) {
					...
					if (curTexAtlas.AsTexArray) {
						var w = curTexAtlas.TexArraySize.x * (propCutInTexArray.boolValue ? bt.countX : 1);
						var h = curTexAtlas.TexArraySize.y * (propCutInTexArray.boolValue ? bt.countY : 1);
						GUILayout.Label("Tex size in atlas: " + w + "x" + h);
					}
					else {
				...
				if ((newCountX != 1 || newCountY != 1) && (curTexAtlas != null && curTexAtlas.AsTexArray)) {
					EditorGUILayout.PropertyField(propCutInTexArray, new GUIContent("Cut To Single Textures"));
				}
