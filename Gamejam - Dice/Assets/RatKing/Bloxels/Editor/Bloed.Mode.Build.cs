	...
	public partial class Bloed : EditorWindow {
		...
		void OnGUI_Build(Color guiNormalColor) {
			...
			if (styleBtnDropDown == null) {
				styleBtnDropDown = new GUIStyle("button") {
			...
			if (ProjectSettings.Types != null) {
				...
				// scroll through the shelves
					...
					idx = (idx + (Event.current.delta.x + Event.current.delta.y > 0 ? 1 : -1) + ProjectSettings.TypeShelves.Count) % ProjectSettings.TypeShelves.Count;
				...
				void TypeButton(BloxelTemplate tmp) {
					...
					if (GUI.Button(new Rect(rect.x + ((ii - 1) % c) * rect.width / c, rect.y, rect.width / c, 30f), display)) {
						if (Event.current.button == 0) {
							curBloxTmp = tmp.Type != null ? tmp.Type.Templates[0] : tmp; // choose the template
							SceneView.RepaintAll();
						}
						else if (tmp.Type != null) {
							Selection.activeObject = tmp.Type; // select the template in the editor
							EditorGUIUtility.PingObject(tmp.Type);
						}
					}
			...
			if (ProjectSettings.Textures != null) {
				...
				// scroll through the shelves
					...
					idx = (idx + (Event.current.delta.x + Event.current.delta.y > 0 ? 1 : -1) + ProjectSettings.TextureShelves.Count) % ProjectSettings.TextureShelves.Count;
				...
				bool TextureButton(int idx) {
					...
					if (GUI.Button(r, GUIContent.none)) {
						if (Event.current.button == 0) {
							curBloxTexIdx = idx; // choose the texture
							SceneView.RepaintAll();
						}
						else {
							Selection.activeObject = ProjectSettings.Textures[idx]; // select the texture in the editor
							EditorGUIUtility.PingObject(ProjectSettings.Textures[idx]);
						}
					}
					else if (Event.current.type == EventType.ScrollWheel && Event.current.shift && r.Contains(Event.current.mousePosition)) {
						textureButtonSize = Mathf.Clamp(textureButtonSize + (Event.current.delta.x + Event.current.delta.y > 0 ? -1 : 1), -3, 5); // make button size smaller or bigger
						Event.current.Use();
						Repaint();
						GUILayout.EndHorizontal();
