...
namespace RatKing {
	...
	public class BloxelJoist : MonoBehaviour {
		...
		public void UpdateMesh() {
			...
			var tas = BloxelUtility.ProjectSettings.TexAtlases[texture.texAtlasIdx];
			var mat = mr.material = tas.Material;
			...
			for (int h0 = 0; h0 <= targetH; ++h0) {
				...
				var ai = texture.arrayIdx[0 + ((v_z + 1) % countY) * countX + ((v_x + 1) % countY)];

				var uv_z_00 = new Vector3(rect.x + u_z_0, rect.y + (v_z * th), ai);
				var uv_z_11 = new Vector3(rect.x + u_z_1, rect.y + (v_z + Mathf.Clamp01(yFactor)) * th, ai);
				var uv_z_01 = new Vector3(uv_z_00.x, uv_z_11.y, ai);
				var uv_z_10 = new Vector3(uv_z_11.x, uv_z_00.y, ai);
					
				var uv_x_00 = new Vector3(rect.x + u_x_0, rect.y + (v_x * th), ai);
				var uv_x_11 = new Vector3(rect.x + u_x_1, rect.y + (v_x + Mathf.Clamp01(yFactor)) * th, ai);
				var uv_x_01 = new Vector3(uv_x_00.x, uv_x_11.y, ai);
				var uv_x_10 = new Vector3(uv_x_11.x, uv_x_00.y, ai);
			...
			if (caps) {
				...
				var ai = texture.arrayIdx[0 + ((v_z + 1) % countY) * countX + ((v_x + 1) % countY)];
				var uv_z_00 = new Vector3(rect.x + u_z_0, rect.y + u_y_0, ai);
				var uv_z_11 = new Vector3(rect.x + u_z_1, rect.y + u_y_1, ai);
				var uv_z_01 = new Vector3(uv_z_00.x, uv_z_11.y, ai);
				var uv_z_10 = new Vector3(uv_z_11.x, uv_z_00.y, ai);
			...
			if (texRotate) {
				for (int i = 0; i < count; ++i) {
					var uv = uvs[i]; uv.x -= center.x; uv.y -= center.y;
					var temp = uv.x; uv.x = uv.y; uv.y = -temp;
					uv.x += center.x; uv.y += center.y; uvs[i] = uv;
				}
			}
