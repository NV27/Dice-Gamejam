...
namespace RatKing.Bloxels {
	...
	public class BloxelLevelSettings : ScriptableObject {
		...
		//
		BloxelTemplate[] standardBloxelsPreparedTemplate = null;
		BloxelTexture[] standardBloxelsPreparedTexture = null;
		...
		public Bloxel GetStandardBloxel(Base.Position3 pos) {
			var idx = standardMethod(pos);
			var tIdx = BloxelUtility.CurLevel.TextureUIDs.IndexOf(standardBloxelsPreparedTexture[idx].ID);
			return new Bloxel(standardBloxelsPreparedTemplate[idx], tIdx);
		}
		...
		public void ResetStandardBloxels() {
			...
			standardBloxelsPreparedTemplate = new BloxelTemplate[standardBloxels.Length];
			standardBloxelsPreparedTexture = new BloxelTexture[standardBloxels.Length];
			for (int i = 0; i < standardBloxels.Length; ++i) {
				var sb = standardBloxels[i];
				standardBloxelsPreparedTemplate[i] = sb.isAir ? ps.AirTemplate : ps.BoxTemplate;
				standardBloxelsPreparedTexture[i] = sb.texture != null ? sb.texture : ps.MissingTexture;
			}
